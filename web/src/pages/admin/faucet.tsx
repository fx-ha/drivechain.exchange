...
import { useFaucetRequestsQuery } from '../../generated/graphql'
...
const FaucetAdmin = () => {
  ...
  return (
    <Layout title="Faucet | Admin | Drivechain Exchange">
      <AdminNav />
      ...
      <Box mt="10" overflowX="auto">
        ...
        <Table maxW="full" variant="simple" size="sm" mb="5">
          <TableCaption placement="top">Faucet Requests</TableCaption>
          <Thead>
            <Tr>
              <Th>ID</Th>
              <Th>Address</Th>
          ...
          <Tbody>
            {data?.faucetRequests.faucetRequests.map(
              ({
                id,
                address,
                amount,
                ...
                <Tr key={id}>
                  ...
                  <Td>{isPaid ? 'x' : ''}</Td>
                  <Td>{amount}</Td>
                  <Td>{txid ? truncateMiddle(txid) : ''}</Td>
        ...
        <Flex w="full" alignItems="center" justifyContent="center">
          <PaginateButton
            disabled={!data?.faucetRequests.hasMore}
            isLoading={loading}
            onClick={() => {
              if (data?.faucetRequests.hasMore) {
                setCursor(
                  data?.faucetRequests.faucetRequests[
                    data.faucetRequests.faucetRequests.length - 1
                  ].createdAt
                )
              }
            }}
